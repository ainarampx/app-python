...
# ============================================================
# ECAP STORE — Dash app (version définitive corrigée)
# ============================================================
...
def frequence_meilleure_vente(data: pd.DataFrame, top: int = 10, ascending: bool = False) -> pd.DataFrame:
    """
    Top produits selon somme Quantity, puis agrégation par Gender.
    Retour: Product_Category, Gender, Quantity
    """
    top_prod = (
    ...
    out = (
        out.groupby(["Product_Category", "Gender"])["Quantity"]
        .sum()
        .reset_index()
    )
...
def update(zone_value):
    ...
    # BAR TOP10
    top_df = frequence_meilleure_vente(dff, top=10, ascending=False)

    order = (
        top_df.groupby("Product_Category")["Quantity"]
        .sum()
    ...
    fig_bar.update_yaxes(title_text="Categorie du produit")
